...
export function KleurenwiezenRoundPanel({ appState, dispatchAction }) {
  ...
  const trickCounterText = useMemo(() => {
    ...
    return `${attackLabel}: ${teamSummary.attackTricks} · ${defenseLabel}: ${teamSummary.defenseTricks}`;
  }, [attackLabel, contract?.teamMode, defenseLabel, teamSummary.attackTricks, teamSummary.defenseTricks]);
  ...
  return (
    <div style={panelStyle({ padding: 16, display: "grid", gap: 14 })}>
      ...
      <div style={{ ...softCardStyle({ padding: 18, display: "grid", gap: 8 }), border: `1px solid rgba(255,255,255,0.12)` }}>
        <div style={{ color: colors.muted, fontSize: 12, textTransform: "uppercase", fontWeight: 700 }}>Live slagenstand</div>
        <div style={{ fontWeight: 900, fontSize: 28, lineHeight: 1.1 }}>{trickCounterText}</div>
        <div style={{ color: colors.muted, fontSize: 14 }}>Gespeeld: {playedTricks} / {totalTricks} · Nog te spelen: {remainingTricks}</div>
      </div>
      ...
      {!slice?.roundFinished && canClaimEarly ? (
        <div style={softCardStyle({ padding: 14, display: "grid", gap: 10 })}>
          ...
          <div style={{ color: colors.muted, fontSize: 13 }}>
            Gebruik dit alleen als iedereen akkoord is wie de resterende slagen krijgt. De app kent die resterende slagen dan handmatig toe en opent meteen het resultaat van de ronde.
          </div>
          {!showClaimChoice ? (
            ...
            <div style={{ display: "grid", gap: 10 }}>
              <div style={{ color: colors.muted, fontSize: 13 }}>Wie krijgt de resterende {remainingTricks} slag{remainingTricks === 1 ? "" : "en"}?</div>
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button onClick={() => handleClaimAward("attack")} style={buttonStyle("primary")}>
                  Geef aan declarant / team
                </button>
                <button onClick={() => handleClaimAward("defense")} style={buttonStyle("secondary")}>
      ...
      {slice?.roundFinished ? (
        <div style={softCardStyle({ padding: 16, display: "grid", gap: 10, border: `1px solid ${evaluation.success ? "rgba(74, 222, 128, 0.28)" : "rgba(248, 113, 113, 0.28)"}` })}>
          ...
          <div style={{ color: colors.muted }}>
            {evaluation.attackLabel} haalde {evaluation.attackTricks} slagen. Doel was {evaluation.targetTricks}.
            {slice?.instantFailTriggered ? " Contract stopte meteen omdat de declarant een verboden slag nam in miserie." : ""}
            {slice?.earlyFinishRemainingTricks ? ` Ronde werd vroeg afgerond en ${slice.earlyFinishRemainingTricks} resterende slag${slice.earlyFinishRemainingTricks === 1 ? "" : "en"} werd${slice.earlyFinishRemainingTricks === 1 ? "" : "en"} toegewezen aan ${slice.earlyFinishAwardedTo === "defense" ? defenseLabel : attackLabel}.` : ""}
          </div>
        ...
        <div style={softCardStyle({ padding: 14, display: "grid", gap: 8 })}>
          ...
          <div style={{ color: colors.muted, fontSize: 13 }}>
            Zodra alle vereiste slagen gespeeld zijn — of zodra een miserie-contract direct faalt — berekent de app automatisch winnaar, contractresultaat en punten volgens de ingestelde contractwaardes.
          </div>
        </div>
      )}
    </div>
