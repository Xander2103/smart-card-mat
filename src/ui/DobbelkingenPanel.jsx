...
function ContractCard({ c, onPick }) {
  return (
    <button
      onClick={() => onPick(c.id)}
      style={{
  ...
  return (
    <div style={{ border: "1px solid #eee", borderRadius: 14, padding: 14 }}>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 10 }}>
        <div style={pillStyle()}>Phase: <b>{appState.phase}</b></div>
        <div style={pillStyle()}>Chooser: <b>{chooserName}</b></div>
        <div style={pillStyle()}>Contract: <b>{appState.contract ?? "-"}</b></div>
        <div style={pillStyle()}>Current: <b>{currentName}</b></div>
        <div style={pillStyle()}>TurnZone: <b>{appState.turnZone ?? "-"}</b></div>
      </div>
        ...
        <div style={{ marginTop: 14, display: "grid", gap: 10 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>
            {DOBBELKINGEN_CONTRACTS.map((c) => (
              <ContractCard key={c.id} c={c} onPick={onChooseContract} />
            ))}
          </div>
        </div>
      )}
